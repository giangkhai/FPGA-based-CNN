...
void conv(uint8_t image_in[HEIGHT][WIDTH], uint8_t image_out[HEIGHT][WIDTH], char kernel[3][3]){
    ...
    #pragma HLS INTERFACE m_axi depth=4096 port=image_out offset=slave bundle=DATA_BUS
    #pragma HLS INTERFACE m_axi depth=4096 port=image_in offset=slave bundle=DATA_BUS
